...
class effectmeasure_plot:
    ...
    def __init__(self,label,effect_measure,lcl,ucl):
        ...
        if ((all(isinstance(item,float) for item in ucl))&(all(isinstance(item,float) for item in effect_measure))):
            ...
            self.df['UCL_dif'] = (pd.to_numeric(self.df['UCL'])) - (self.pd.to_numeric(df['OR']))
        self.em = 'OR'
        self.ci = '95% CI'
        self.scale = 'log'
        self.center = 1
...
def func_form_plot(df,outcome,var,f_form=None,outcome_type='binary',link_dist=None,ylims=None,loess_value=0.5,legend=True,model_results=True,loess=True,points=False):
    ...
    outcome_type needs to be changed, in addition to the link_dist. The input dataframe should 
    only contain variables needed for the functional form assessment. Missing observations are dropped.
    A warning is produced about how many observations were dropped when fitting the functional
    form.
